...
2. **Всегда явный вызов** — пайплайн не включается молча, только через триггер-сообщение.
3. **Состояние на диске** — JSON-файл для `/resume` после перезапуска сессии.
